...
void MainWindow::init(int id,QString account,QString password)
{
    ...
    //设置页面
    {
        ...
        connect(btn_group_set,&QButtonGroup::buttonClicked,this,[=](QAbstractButton* btn){
            if(btn==ui->btn_custom){
                ...
                sql_query_factory->newSql("/@",[=](QStringList& list)mutable{
        ...
        //服务器设置页面
        {
            ...
            auto relogin=[=]()->int{
                ...
                sql_query_factory->newSql(content,sqlFunc);
    ...
    connect(ui->btn_friend_refresh,&AutoRotateButton::safeClicked,this,[=](){
        updateFriendList();
    });
    ...
    connect(ui->btn_group_refresh,&AutoRotateButton::safeClicked,this,[=](){
        updateGroupList();
    });
    ...
    //user_patcher_factory->patchUser(m_user,true,[=](User user_patched){
    //    m_user = user_patched;
    //    ui->btn_user->setToolTip(m_user.nickname);
    //    ui->btn_user->setIcon(m_user.icon);
    //});

    emit initProgress(0.4f,"拉取好友列表");
    ...
    //加载好友列表
    {
        ...
        sql_query_factory->newSql(sql.toUtf8(), [=,&progress](QStringList& list){
            ...
            for(int i=2;i<list.size();i++){
                ...
                progress += per_one;
                emit initProgress(progress,QString("拉取好友列表%1/%2").arg(i-1).arg(sum));
            }
            emit initProgress(progress,QString("加载好友列表"));

            // qDebug()<<"当前线程id"<<QThread::currentThreadId();
            QMetaObject::invokeMethod(this,[=](){
                updateFriendList();
                qDebug()<<"加载"<<m_user.friends.size()<<"个好友的历史记录";
                for(auto& user:m_user.friends){
                    loadHistoryMessage(user);
                }
            },Qt::QueuedConnection);
        });
    }
    emit initProgress(progress,"拉取群聊列表");
    //加载群聊
    {
        ...
        sql_query_factory->newSql(sql.toUtf8(),[=,&progress](QStringList& list){
            ...
            for(int i=2; i<list.size(); i++){
                ...
                progress += per_one;
                emit initProgress(progress,QString("拉取群聊列表%1/%2").arg(i-1).arg(sum));
            }
            emit initProgress(progress,QString("加载群聊列表"));
            QMetaObject::invokeMethod(this,[=](){
                // qDebug()<<"加载"<<m_user.friends.size()<<"个群聊的历史记录";
                // for(auto& group:m_user.groups){
                //     loadHistoryMessage(group);
                // }
                updateGroupList();
            },Qt::QueuedConnection);
        });
...
void MainWindow::showTip(const QString &tip)
{
    ...
    if (queue_tip.size() > 100) {
        aniamtion_tip->setDuration(50);
    }
    else if (queue_tip.size() > 10) {
        aniamtion_tip->setDuration(150);
    }
    else {
        aniamtion_tip->setDuration(300);
    }
...
void MainWindow::addMsg(OBJ::Type type, Message msg, bool isHistory)
{
    ...
    if(type == OBJ::user){
        ...
        if(id_receiver == m_user.id){
            id_other = id_sender;
        }else if(id_sender == m_user.id){
            id_other = id_receiver;
        }else{
            return;
        }
        auto add = [=](ChatPreviewButton* btn){
            User user_sender;
            user_sender.id = id_sender;
            ...
            if(btn->isChecked() && ui->stackedWidget_main->currentWidget() == ui->page_chat){
                ...
                mutex_chat.lock();
                map_userchat_unread[id_other].append(msg);
                mutex_chat.unlock();
        ...
        if(!had_preview_btn){
            User user;
            user.id = id_other;
            auto btn = addPreviewButton(user);
            add(btn);
        }
    }else if(type == OBJ::group){
        //群聊消息的接收方一定是群聊
        int id_receiver = msg.receiver_group.id;
        auto add = [=](ChatPreviewButton* btn){
            User user_sender;
            user_sender.id = id_sender;
            Group group_receive;
            group_receive.id = msg.receiver_group.id;
            ...
            if(btn->isChecked() && ui->stackedWidget_main->currentWidget() == ui->page_chat){
                ...
                mutex_chat.lock();
                map_groupchat_unread[id_receiver].append(msg);
                mutex_chat.unlock();
        ...
        if(!had_preview_btn){
            Group group;
            group.id = id_receiver;

            auto btn = addPreviewButton(group);
            add(btn);
        }
    }
}
...
void MainWindow::loadHistoryMessage(OBJ& obj)
{
    // auto read = [=](QTcpSocket* socket)mutable{

    // };
    auto type = obj.getType();
    if(type == OBJ::user){
        QString sql = QString("/m/chat_history_of_friend*%1**%2*").arg(m_user.id).arg(obj.id);
        sql_query_factory->addTask(sql.toUtf8(),[=](QTcpSocket* socket)mutable{
            //这里为QTcpSocket::readyRead的槽函数，socket传入后就不会再主动获取数据流，如果此次处理时socket未能获取到全部的数据，它后续也不会主动获取了，所以需要用socket->waitForReadyRead等待完整数据
            int count;
            QByteArray first_packet, buffer;
            QDataStream in(&buffer,QIODevice::ReadOnly);
            // QDataStream in(socket);
            in.setVersion(QDataStream::Qt_5_15);
            while(true){
                ...
                in>> first_packet;
                if(in.commitTransaction()){
                    // qDebug()<<"读取到first_packet"<<first_packet<<"bytesAvailable"<<socket->bytesAvailable();
                    QStringList list = QString(first_packet).split("/");
                    if(list.size() < 2){
            ...
            qDebug()<<"加载到一组历史记录,个数"<<count;
            // in.device()->seek(0);
            QByteArray packet, data;
            // QDataStream in2(socket);
            // in2.setVersion(QDataStream::Qt_5_15);
            while(count > 0){
                ...
                buffer += socket->readAll();
                static int t=0;
                qDebug()<<"第" <<++t <<"次尝试读取 buffer.size"<<buffer.size()
                         <<"packet.size"<<packet.size()<<"data.size"<<data.size();
                in.startTransaction();
                ...
                if(in.commitTransaction()){
                    ...
                    if(list[1] == "s"){
                        ...
                        QDateTime time = QDateTime::fromString(list[5]);
        ...
        QString sql = QString("/m/chat_history_of_group*%1*").arg(obj.id);
        sql_query_factory->addTask(sql.toUtf8(),[=](QTcpSocket* socket){
            while(socket && socket->size() > 0){
                QByteArray data = defaultReadSocket(socket);
                int counter = 0;
                while(data == "failed"){
                    data = defaultReadSocket(socket);
                    if(counter++>1000){
                        qDebug()<<"chat_history_of_friend: 读取失败";
                        return;
                    }
                }
                // qDebug()<<"loadHistoryMessage:"<<data;
                QStringList list = QString(data).split("/");
                if(list.size() < 2){
                    qDebug()<<"loadHistoryMessage:newSql list"<<list;
                    return;
                ...
                if(list[1] == "s"){
                    ...
                    if(msg_type == "text"){
                        ...
                        int receiver = list[4].toInt();
                        QDateTime time = QDateTime::fromString(list[5]);
                        QString content = list[6];
                        qDebug()<<"msg_type"<<msg_type<<"sender"<<sender<<"receiver"<<receiver
                        ...
                        msg.type = Message::Text;
                        msg.sender = user_sender;
                        msg.receiver_group = user_receiver;
                        msg.time = time;
                        msg.msg = content.toUtf8();

                        QMetaObject::invokeMethod(this,[=](){
                            addMsg(OBJ::group, msg, true);
                        });
                    }
...
void MainWindow::addUserDetailWidget(User user)
{
    ...
    // qDebug()<<"查看用户详情：m_user"<<m_user.id<<"user"<<user.id;
    bool isMyself = (user==m_user);
    if(isMyself){
        isfriend=true;
    }else{
    ...
    // qDebug()<<"isMyself"<<isMyself;
    UserDetail* w=new UserDetail(user,ui->page_user_detail, isfriend, isMyself);
...
void MainWindow::addGroupDetailWidget(Group group)
{
    ...
    connect(w,&GroupDetail::updateGroupList,w,[=](){
        ...
        sql_query_factory->newSql(sql.toUtf8(),[=](QStringList& list){
            for(int i=2; i<list.size(); i++){
                Group group;
                group.id=list[i].toInt();
                addGroup(group,false);
...
bool MainWindow::addFriend(User user_friend,bool save_to_sql)
{
    ...
    if(!had){
        ...
        if(save_to_sql){
            ...
            sql_query_factory->newSql(content,[=](QStringList&){
                return true;
            });
...
{
    if(!mutex_updateFriendList.tryLock())return;
    QList<QWidget*> list_will_delete;
    for(QObject*oj:ui->scrollAreaWidgetContents_friend->children()){
        ChatPreviewButton* w=dynamic_cast<ChatPreviewButton*>(oj);
        if(w){
    ...
    qDebug() << "updateFriendList加载好友，数量：" << m_user.friends.size();
    ...
    if(vl){
        for (User& user_friend : m_user.friends) {
            ...
            preview->unique_text = user_friend.state;
            preview->updateState();
            vl->insertWidget(vl->count()-1, preview);
            connect(preview, &ChatPreviewButton::clicked, preview, [=]() {
                addUserDetailWidget(preview->m_user);
                });
        }
    ...
    mutex_updateFriendList.unlock();
}

void MainWindow::updateGroupList()
{
    if(!mutex_updateGroupList.tryLock())return;
    QList<QWidget*> list_will_delete;
    ...
    qDebug() << "updateGroupList加载群聊，数量：" << m_user.groups.size();
...
void MainWindow::createGroup()
{
    ...
    connect(btn,&QPushButton::clicked,this,[=](){
        ...
        if(!name.isEmpty()){
            ...
            sql_query_factory->newSql(sql,[=](QStringList& list){
                showTip("创建成功");
                Group newGroup;
